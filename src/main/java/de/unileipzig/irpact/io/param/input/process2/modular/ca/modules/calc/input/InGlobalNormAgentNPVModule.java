...
import static de.unileipzig.irpact.io.param.input.TreeViewStructureEnum.PROCESS_MODEL4_PVACTMODULES_NUMBERINPUT_InGlobalNormAgentNPVModule;
...
@LocalizedUiResource.PutClassPath(PROCESS_MODEL4_PVACTMODULES_NUMBERINPUT_InGlobalNormAgentNPVModule)
public class InGlobalNormAgentNPVModule implements InConsumerAgentInputModule2 {
...
//        setShapeColorFillBorder(res, thisClass(), INPUT_SHAPE, INPUT_COLOR, INPUT_COLOR, INPUT_BORDER);
    ...
    public InGlobalNormAgentNPVModule() {
    }

    public InGlobalNormAgentNPVModule(String name) {
        setName(name);
    }

    public InGlobalNormAgentNPVModule(String name, InPVFile pvFile) {
        setName(name);
        setPvFile(pvFile);
    }

    @Override
    public InGlobalNormAgentNPVModule copy(CopyCache cache) {
        return cache.copyIfAbsent(this, this::newCopy);
    }

    public InGlobalNormAgentNPVModule newCopy(CopyCache cache) {
        InGlobalNormAgentNPVModule copy = new InGlobalNormAgentNPVModule();
        return Dev.throwException();
    }

    @Override
    public GlobalNormAgentNPVModule parse(IRPactInputParser parser) throws ParsingException {
        ...
        GlobalNormAgentNPVModule module = new GlobalNormAgentNPVModule();
    ...
    private void applyPvFile(IRPactInputParser parser, GlobalNormAgentNPVModule module) throws ParsingException {
        if(hasPvFile()) {
