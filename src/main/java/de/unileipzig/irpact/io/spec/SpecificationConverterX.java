 ...
 * @author Daniel Abitz
 */
@SuppressWarnings({"unused", "MismatchedQueryAndUpdateOfCollection", "ConstantConditions", "UnnecessaryLocalVariable"})
@Todo("AWARENESS")
@Todo("fixen, siehe die todos")
public class SpecificationConverterX {

    @SuppressWarnings("rawtypes")
    protected final Map<Class, ToSpecFunction> toSpecMap = new HashMap<>();

    public SpecificationConverterX() {
        initDefaults();
    }
...
//        root.version = inVersionToParam.toParam(manager, root.version, cache);
    ...
    public interface ToSpecFunction<T> {
        ...
        void toJson(T instance, SpecificationManager manager, SpecificationConverterX converter);
    }
