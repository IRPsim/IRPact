 ...
 * @author Daniel Abitz
 */
public class AnnualNormExistingAssetNPVModule
        extends AbstractCACalculationModule2
        implements RAHelperAPI2 {

    private static final IRPLogger LOGGER = IRPLogging.getLogger(AnnualNormExistingAssetNPVModule.class);
    ...
    public double calculate(ConsumerAgentData2 input, List<PostAction2> actions) throws Throwable {
        ...
        double min = dataSupplier.annualMinExistingAssetNPV(input.streamConsumerAgents(), getCurrentYear(input));
        double max = dataSupplier.annualMaxExistingAssetNPV(input.streamConsumerAgents(), getCurrentYear(input));
