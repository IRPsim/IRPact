 ...
 * @author Daniel Abitz
 */
public class NeighbourhoodOverview extends AbstractGeneralDataHandler {

    private static final IRPLogger LOGGER = IRPLogging.getLogger(NeighbourhoodOverview.class);

    public NeighbourhoodOverview(DataProcessor4 processor, String baseName) {
    ...
    protected List<ConsumerAgent> listNeighbours(ConsumerAgent source, Product product, NodeFilter filter) {
        return processor.getRAHelperAPI()
                .streamFeasibleAndFinancialNeighbours(getEnvironment(), source, product, filter)
