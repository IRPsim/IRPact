...
import de.unileipzig.irpact.commons.util.data.map.Map4;
...
public class BasicDataAnalyser implements DataAnalyser {
    ...
    private final Map4<Integer, Product, ConsumerAgent, AgentDataState> agentStateData = Map4.newConcurrentHashMap();

    private AgentDataState getData(ConsumerAgent agent, Product product, int year) {
        return agentStateData.computeIfAbsent(year, product, agent, _agent -> new AgentDataState());
    }

    @Override
    public AgentDataState getAgentDataState(ConsumerAgent agent, Product product, int year) {
        return getData(agent, product, year);
    }

    @Override
    public void logAgentDataState(ConsumerAgent agent, Product product, int year, AgentDataState state) {
