    ...
    public void body() {
        ...
        System.out.println(
                agent.getFeature(IRequiredServicesFeature.class)
                        .searchServices(new ServiceQuery<>(KillPlatformService.class, ServiceScope.NETWORK))
                        .get()
                        .size());
        agent.getFeature(IRequiredServicesFeature.class)
                .searchServices(new ServiceQuery<>(KillPlatformService.class, ServiceScope.NETWORK))
                .addResultListener(new IntermediateDefaultResultListener<KillPlatformService>() {
                    @Override
