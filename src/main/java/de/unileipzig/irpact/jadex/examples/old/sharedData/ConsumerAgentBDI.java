...
public class ConsumerAgentBDI {
    ...
    protected void body() {
        ...
        logger.debug("{} 1. add: {}", sayMyName(), wait1);
        execFeature.waitForDelay(wait1, ia -> {
            bdiFeature.dispatchTopLevelGoal(new AddProductGoal(name + "#" + wait1)).get();
            return IFuture.DONE;
        });

        long wait2 = rnd.nextInt(2000) + wait1;
        logger.debug("{} 2. add: {}", sayMyName(), wait2);
        execFeature.waitForDelay(wait2, ia -> {
            bdiFeature.dispatchTopLevelGoal(new AddProductGoal(name + "#" + wait2)).get();
            return IFuture.DONE;
        });
    }
